...
from dataset.dscovr.all import All
import textwrap


def dscovr_open(filepath):
    check_value_type(filepath, str, "dscovr_open")
    check_file(filepath, "dscovr_open")
    ...
    with open(filepath, 'r') as f:
        ...
        if "bx" in data_list and "by" in data_list :
            dscovr = DSCOVR_Mag(filepath)

        elif "density" in data_list and "speed" in data_list :
            dscovr = DSCOVR_Plasma(filepath)
        else :
...
#        header = self._add_info_to_header(hd, data)
        ...
        self.header = header
        self.data = data
        self.all = All(header_lines, data_lines)
...
#        check_value_type(header, Header, "info")
    ...
    def _parsing_data(self, data_lines, names, formats):
        ...
        for line in data_lines:
            ...
            values = line.split()
            date, time = values.pop(0), values.pop(0)
            time = time[:5] ## is it necessary? 
...
class DSCOVR_Mag(DSCOVR):
    ...
    def __init__(self, filepath):
        names = ("Date", "bx_gsm", "by_gsm", "bz_gsm", "lon_gsm", "lat_gsm", "bt")
        formats = ("datetime64[s]", "f8", "f8", "f8", "f8", "f8", "f8")
        
        super().__init__(filepath, names, formats)


class DSCOVR_Plasma(DSCOVR):
    """
    Parameters
    ----------
    filepath: str
    """
    def __init__(self, filepath):
        names = ("Date", "density", "speed", "temperature")
        formats = ("datetime64[s]", "f8", "f8", "i8")
