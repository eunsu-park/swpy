...
from dataset.ace.all import All
import textwrap

def ace_open(filepath):
    check_value_type(filepath, str, "ace_open")
    check_file(filepath, "ace_open")
    ...
    with open(filepath, 'r') as f:
        ...
        print(data_list)
        if("mag" in data_list):
            ace = ACE_MAG(filepath)
        elif("sis" in data_list):
            ace = ACE_SIS(filepath)
        elif("swepam" in data_list):
            ace = ACE_SWEPAM(filepath)
        else:
            raise Exception(textwrap.dedent("""\
                                            Can't determine ace data type.
...
class ACE_MAG(ACE):
    ...
    def __init__(self, filepath):
        names = ("Date", "Status", "Bx", "By", "Bz", "Bt", "Latitude", "Longitude")
        formats = ("datetime64[s]", "i2", "f8", "f8", "f8", "f8", "f8", "f8")
...
class ACE_SIS(ACE):
    ...
    """
    def __init__(self, filepath):
        names = ("Date", "Status_10", "MeV_10", "Status_30", "MeV_30")
        formats = ("datetime64[s]", "i2", "f8", "i2", "f8")
        
        super().__init__(filepath, names, formats)


class ACE_SWEPAM(ACE):
    """
    Parameters
    ----------
    filepath: str
    """
    def __init__(self, filepath):
        names = ("Date", "Status", "ProtonDensity", "BulkSpeed", "IonTemperature")
        formats = ("datetime64[s]", "i4", "f8", "f8", "f8")
